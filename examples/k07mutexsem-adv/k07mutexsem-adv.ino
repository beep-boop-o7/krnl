...
int saveDataInCritRegionNoWait(int v)  // pick up data from critical region protected by a semaphore
{
	int x;
	x = k_wait(semmutex, -1);
...
struct shDataTp getDataInCritRegion(void)
{
	...
	k_eat_msec(70);  // to block region some time so the other cant get it at once
	sharedData.counter--;
	tmp = sharedData;
